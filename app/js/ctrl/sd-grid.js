        ...
        link: function (scope, el, attr) {
          ...
          var loading = function (jq, cp, ht) {
            var body = jq.find('table tbody').empty().removeClass('tr-link'),
              tr = $('<tr class="loading">'
                + '<td colspan="' + cp + '"><div><img src="img/loading.gif"></div>'
          ...
          var $loading = angular.element('<tr class="loading">'
              + '<td colspan="' + columnLen + '"><div><img src="img/loading.gif"></div>'
              + '<div class="tip">正在加载...</div></td>'
              + '</tr>'),
            $noResult = angular.element('<tr style="display:none;" class="none no-result">'
          ...
          var loadPage = function (page) {
            ...
            $http({url: option.url, method: option.method || 'POST', data: data})
              .success(function (dt) {
                if (dt.state === true) {
                  var d = dt.data;
                  $grid.total = d.totalRec;
                  totalPage = Math.ceil(parseInt(d.totalRec) / parseInt($grid.size));
          ...
          footEle.prototype = {
            ...
            hide: function () {
              ...
              angular.element(this[0]).hide();
            },
            show: function () {
              //angular.element(this[0]).parent().show();
              angular.element(this[0]).show();
            },
            text: function (t) {
              angular.element(this[0]).text(t);
            }
          };
          var btns = $gridFoot.find('a');
          var eles = {
          ...
          angular.element('.advanced-search .search-content-wrapper').bind('click', function (e) {
            e.stopPropagation();
          });
           ...
           */
            .bind('click', function (e) {
              ...
              if (ck.hasClass('search-label') || ck.hasClass('ui-menu-item')) {
                return;
              }
              if ($grid.advanced) {
                $grid.advanced = false;
